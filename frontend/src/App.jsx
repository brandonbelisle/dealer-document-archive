...
// ── Subscriptions modal state ──────────────────────────
  ...
  const [initialRepairOrderSlsId, setInitialRepairOrderSlsId] = useState(null);
  const [dcvInitialTab, setDcvInitialTab] = useState("timeline");
...
const scanWatchedFolder = useCallback(async () => {
  if (!watchedFolderHandle || !watchFolderEnabled) {
    console.log('Scan skipped: no watch folder handle or not enabled');
    return;
  }
  ...
  try {
    ...
    if (permission !== 'granted') {
      console.log('Watch folder permission not granted');
      setIsScanning(false);
      return;
    }
...
useEffect(() => {
  if (autoUploadEnabled && watchedFiles.length > 0 && !isScanning && !uploadingRef.current) {
    console.log('Triggering auto-upload for', watchedFiles.length, 'files');
    const filesToUpload = [...watchedFiles];
    const timer = setTimeout(() => {
      autoUploadWatchedFiles(filesToUpload);
    }, 500);
...
const handleDeptDrop = useCallback(async (e) => {
    ...
    // Collect all entries synchronously BEFORE any async operations

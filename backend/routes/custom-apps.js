...
router.get('/', requireAuth, async (req, res) => {
  try {
    const [rows] = await db.execute(
      'SELECT id, name, link, created_at FROM custom_apps ORDER BY name'
    );
    res.json(rows);
  } catch (err) {
    console.error(err);
...
router.post('/', requireAuth, requirePermission('manageSettings'), async (req, res) => {
  try {
    ...
    const id = uuidv4();
    await db.execute(
      'INSERT INTO custom_apps (id, name, link, created_by) VALUES (?, ?, ?, ?)',
      [id, name.trim(), link.trim(), req.user.id]
    );
...
router.put('/:id', requireAuth, requirePermission('manageSettings'), async (req, res) => {
  try {
    ...
    await db.execute(
      'UPDATE custom_apps SET name = ?, link = ? WHERE id = ?',
      [name.trim(), link.trim(), req.params.id]
    );

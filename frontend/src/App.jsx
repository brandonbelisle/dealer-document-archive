...
const t = getTheme(darkMode);
  ...
  useEffect(() => { if (!isLoggedIn || page !== "admin") return; api.getGroups().then((groups) => { setSecurityGroups(groups.map((g) => ({ id: g.id, name: g.name, desc: g.description, permissions: g.permissions, memberCount: g.memberCount }))); }).catch(console.error); }, [isLoggedIn, page]);
  useEffect(() => { if (!isLoggedIn || page !== "admin" || adminSection !== "users") return; api.getUsers().then((users) => { setAdminUsers(users.map((u) => ({ name: u.display_name, email: u.email, groups: u.groups || [], status: u.status === "active" ? "Active" : "Inactive", id: u.id, groupIds: u.groupIds || [] }))); }).catch(console.error); }, [isLoggedIn, page, adminSection]);
